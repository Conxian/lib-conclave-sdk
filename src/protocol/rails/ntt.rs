...
impl SovereignRail for NTTRail {
    fn name(&self) -> String { "NTT".to_string() }
    ...
    fn validate_request(&self, request: &SwapRequest) -> Result<Option<String>, String> {
        ...
        Ok(Some(format!("NTT_MGR_{}_TO_{}", request.from_asset.chain, request.to_asset.chain)))
    }
...
mod tests {
    ...
    async fn test_ntt_validation_success() {
        ...
        let request = SwapRequest {
            from_asset: AssetIdentifier { chain: "ETH".to_string(), symbol: "USDC".to_string() },
            to_asset: AssetIdentifier { chain: "ARBITRUM".to_string(), symbol: "USDC".to_string() },
            amount: 1000000,
    ...
    async fn test_ntt_validation_different_assets_fail() {
        let ntt = NTTRail;
        let request = SwapRequest {
            from_asset: AssetIdentifier { chain: "ETH".to_string(), symbol: "ETH".to_string() },
            to_asset: AssetIdentifier { chain: "ARBITRUM".to_string(), symbol: "USDC".to_string() },
            amount: 1000000,
            recipient_address: "0x1234567890123456789012345678901234567890".to_string(),
            attribution: None,
    ...
    async fn test_ntt_via_rail_proxy() {
        ...
        let request = SwapRequest {
            from_asset: AssetIdentifier { chain: "BASE".to_string(), symbol: "USDC".to_string() },
            to_asset: AssetIdentifier { chain: "ETH".to_string(), symbol: "USDC".to_string() },
            amount: 5000000,
            recipient_address: "0x1234567890123456789012345678901234567890".to_string(),
            attribution: None,
        };
    ...
    fn test_asset_registry_l2_support() {
        ...
        // Test validation
        let eth_usdc = AssetIdentifier { chain: "ETH".to_string(), symbol: "USDC".to_string() };
        let arb_usdc = AssetIdentifier { chain: "ARBITRUM".to_string(), symbol: "USDC".to_string() };
